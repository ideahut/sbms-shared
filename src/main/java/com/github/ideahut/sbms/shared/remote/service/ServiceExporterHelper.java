...
public abstract class ServiceExporterHelper {
	 ...
	 * GET METHOD
	 */
	public static Method getMethod(Object handler, HttpServletRequest request) throws Exception {
		Method method = null;
	 ...
	 */
	public static List<ServiceExporterInterceptor> prepareInterceptors(List<ServiceExporterInterceptor> interceptors) {
		if (interceptors == null) {
			return new ArrayList<ServiceExporterInterceptor>();
		}
		List<ServiceExporterInterceptor> tempInterceptors = new ArrayList<ServiceExporterInterceptor>(interceptors);
		interceptors.clear();
		while (!tempInterceptors.isEmpty()) {
			ServiceExporterInterceptor interceptor = tempInterceptors.remove(0);
			if (interceptor == null || interceptors.contains(interceptor)) continue;
			interceptors.add(interceptor);
		}
		tempInterceptors = null;
		return interceptors;
	}
